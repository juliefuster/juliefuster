...
import { Building2, ArrowLeft, CheckCircle, AlertCircle, Clock } from "lucide-react"
...
export default function RoomInspectionPage() {
  ...
  const hotelName =
    hotel.toLowerCase() === "caledonian" ? "Hotel Caledonian" : "Hotel Chi"
    ...
    for (const room of roomsList) {
      ...
      if (!error && data && data.length > 0) {
        const allCorrect = data.every(
          (item) => item.status === "correcto" || item.status === "reparado"
        )
  ...
  const floors = Array.from(
    new Set(
      rooms.map((r) => r.floor).filter((f) => f !== null && f !== undefined && !isNaN(f))
    )
  ).sort((a, b) => a - b)
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        {loading ? (
          ...
          <>
            ...
            <Tabs
              value={selectedFloor}
              onValueChange={(v) => setSelectedFloor(v)}
            >
              ...
              <Card className="p-4 flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-2xl font-bold text-slate-900">
                    {Object.values(roomStatuses).filter(
                      (s) => s.status === "complete"
                    ).length}
                  </p>
                </div>
